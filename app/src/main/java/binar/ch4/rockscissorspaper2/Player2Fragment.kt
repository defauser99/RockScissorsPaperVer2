...
import binar.ch4.rockscissorspaper2.databinding.FragmentPlayer2Binding
...
class Player2Fragment : Fragment() {
    ...
    private fun playerTwoGame() {
        binding.btnRockP2.setOnClickListener {
            playerTwoSelect(it)
            if (option1 != "") {
                checkResult()
            }
        }
        binding.btnPaperP2.setOnClickListener {
            playerTwoSelect(it)
            if (option1 != "") {
                checkResult()
            }
        }
        binding.btnScissorsP2.setOnClickListener {
            playerTwoSelect(it)
            if (option1 != "") {
                checkResult()
            }
    ...
    private fun playerTwoSelect(view: View){
        if (!state2) {
            ...
            view.background = ResourcesCompat.getDrawable(activity!!.resources, R.drawable.bg_selected, null)
            Toast.makeText(activity, "Player 2 memilih ${view.contentDescription}",Toast.LENGTH_LONG).show()
        } else {
            refreshPlayerTwo()
        }
    }
    ...
    private fun checkResult(){
        ...
        when (option1) {
            "batu" -> {
                when (option2) {
                    "kertas" -> {
                        showCustomDialog("Player 2 MENANG")
                    }
                    "gunting" -> {
